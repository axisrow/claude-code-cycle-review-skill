...
1. **Start Codex first (background) when `@codex` is configured**, so it runs while `/review` works:
   - **Resolve the companion path with `resolve-companion.sh`** (beside this `SKILL.md`) — manual override first, then the installed-plugins manifest; never hardcode the source namespace or path layout. Do not re-derive the resolution rules here; the script is the spec (its header documents the override precedence, the `scope: "user"` filter, the anchored `^codex(-fork)?$` name allowlist, the `lastUpdated`-descending order, and the two probed path layouts). Plain shell, no network — this call does not itself need `dangerouslyDisableSandbox`; only the companion invocation further below does:
     ```bash
     RESOLVED=$(bash "<path-to-skill-dir>/resolve-companion.sh")
     COMPANION=""
     OVERRIDE_BROKEN=false
     case "$RESOLVED" in
       "OVERRIDE_BROKEN "*) OVERRIDE_BROKEN=true; BROKEN_PATH="${RESOLVED#OVERRIDE_BROKEN }" ;;
       NOT_FOUND)           ;;
       *)                   COMPANION="$RESOLVED" ;;
     esac
     ```
     The three outcomes are distinct and must be handled distinctly in step 6:
     - **a path** — use it as `$COMPANION`.
     - **`OVERRIDE_BROKEN <path>`** — `CODEX_COMPANION_PATH` (or the deprecated `CYCLE_REVIEW_CODEX_COMPANION_PATH`) is set but names no existing file. This **fails closed**: the script deliberately did NOT consult the manifest, so do not run a manifest-found companion instead — that would silently execute an executable the user didn't name and hide their typo. Surface `<path>` and stop.
     - **`NOT_FOUND`** — no override, and the manifest has no user-scoped codex install with a companion on disk. Plugin-not-found fail-closed case in step 6.
   ...
   - **Invoke the companion in the background, JSON output, against the PR base:**
     ```bash
     node "$COMPANION" adversarial-review --background --json --base "$ROUND_BASE_SHA" $CODEX_FLAGS \
     ...
     ```
     Run via **`Bash` with `run_in_background: true`** *and* `dangerouslyDisableSandbox: true` (Codex needs network). **`--background` here selects the companion's turn budget, not detached execution.** For `review`/`adversarial-review` the companion (upstream 1.0.5 and codex-fork alike) always blocks in-process and prints the final `--json` object to stdout — only its `task` command actually enqueues a detached worker — so the background handle still completes only when Codex is done. The flag matters on the fork: without it, the fork applies its 110s *foreground* turn budget (sized to duck under a host Bash-tool SIGKILL that never happens here, since the bash is already backgrounded), which kills any complex diff at high effort; with it, the run gets the background budget (600s idle timeout + 45-min hard wall-clock ceiling — the companion still bounds a hung run itself, so no extra cap is needed on our side). On upstream codex the flag is parsed and ignored (no turn timeouts there) — safe for both. Do NOT use `--wait`: it is a no-op for review commands (they always block) and leaves the fork on the 110s budget. If a future companion version turns review `--background` into a real enqueue, stdout becomes `{jobId, status: "queued", …}` with no `.result` — step 4.4's no-parseable-`.result` fail-closed stops the round (safe degradation; revisit then). **Record the returned background shell id** as `$CODEX_SHELL_ID` — it is consumed by the live-progress Monitor in step 4.3, which streams Codex's stderr to the user while it runs (Codex otherwise runs silently in the background).
...
4. **Fail-closed check (Codex configured but did not return a verdict).** **Detection is one rule, cause-agnostic: if Codex returned no parseable `.result` this round — companion missing, non-zero exit, or exit-0 with empty/`.parseError`/absent `.result` — it is silent, and silence is not approval** (root invariant in `## Important`). Every silent shape stops the same way; do NOT continue on `/review` alone. The cause only shapes the *user-facing message*:
   - **Companion not found** (`$COMPANION` empty): if `OVERRIDE_BROKEN=true` (the resolver returned `OVERRIDE_BROKEN <path>`), tell the user their companion override is set but points at a file that doesn't exist, quoting `$BROKEN_PATH`, and stop — the manifest resolver was deliberately **not** consulted (fail-closed on the override, not a fallback), so do not run a manifest-found companion instead. The usual cause is a plugin version bump leaving the pinned path behind: the fix is to update (or unset) `CODEX_COMPANION_PATH` in their shell profile. Otherwise (`NOT_FOUND`, override unset): the codex plugin is not installed at user scope and no override was set: tell the user both remediation paths in one message — "Codex plugin not found. If you installed it via `/plugin`, check it's user-scoped (not local/project). If you're running a manual/dev install not registered in `installed_plugins.json`, set `CODEX_COMPANION_PATH=/path/to/codex-companion.mjs` in your shell profile and re-run." Do **NOT** suggest `codex login` here — nothing to log in to yet.
