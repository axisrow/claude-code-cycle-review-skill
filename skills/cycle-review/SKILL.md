...
```
For example, with both configured the body starts with `@claude @codex review PR #<PR> at exact head <ROUND_HEAD_SHA>.`. Run via `Bash` with `dangerouslyDisableSandbox: true`. **Capture the request comment's ID and the round-start timestamp** (step 6 needs them for round-bound completion checks):
```bash
REQUEST_COMMENT_ID=$(gh pr comment <PR> --body "<MENTIONS> review PR #<PR> at exact head <ROUND_HEAD_SHA>. ..." | grep -oE '[0-9]+$')
ROUND_START_TS=$(date -u +%Y-%m-%dT%H:%M:%SZ)
```
Then go to step 5 (wait).

**Attestation (applied in step 6):** GitHub review objects and inline comments carry a `commit_id`. Step 6 accepts, as *this round's* reviewer output, only **new** objects (`id > floor`) whose `commit_id == ROUND_HEAD_SHA`. Issue comments have no `commit_id` — they may supply findings (re-verified against `ROUND_HEAD_SHA`) and **may count as a reviewer completion through the round-bound timestamp check** (`claude[bot]` issue comment with `updated_at > ROUND_START_TS` = heard from).
...
| `@codex` | `@codex` | `chatgpt-codex-connector[bot]` | a **PR review** object (`### 💡 Codex Review`) + inline comments with severity badges (`P1`/`P2`) | **absence** of review-object — reacts with 👍 instead (no comment = clean) |
...
Read all comments and review comments from **all reviewers** (bot and human), **attested to this round's `ROUND_HEAD_SHA`**. Review objects and inline comments carry `commit_id`; accept only **new** ones (`id > floor`) with `commit_id == ROUND_HEAD_SHA`. Issue comments have no `commit_id` — they supply findings (re-verified against `ROUND_HEAD_SHA`) and **may count as a reviewer completion through the round-bound timestamp check** (`claude[bot]` issue comment with `updated_at > ROUND_START_TS`). Fetch under **`set -euo pipefail`** so that a failed `gh api` **aborts the whole triage** (not just sets a non-zero status that later commands ignore). Without `set -e`, `pipefail` alone lets a later successful fetch mask an earlier failure → silently dropped surface → merge authorizes without critical findings:
```bash
set -euo pipefail
...
- **(cloud only) No reviewer was actually heard from this round** → do NOT finalize. If the configured bots posted nothing new for this round (the bots were slow, or Claude shows a usage-limit message instead of a review and no other reviewer responded), then no review happened — "no `FIX` verdicts" only reflects silence, not an approval. **How to recognize each bot's review** (studied from real PR comments — these are the markers to look for in step 6's fetched comments/reviews):
  ...
  - **`chatgpt-codex-connector[bot]`** posts a **PR review object** (`state=COMMENTED`) with the heading `### 💡 Codex Review` + inline comments carrying severity badges (`P1`/`P2`/etc). A **clean** Codex review is the **absence** of a review-object — Codex reacts with 👍 on the PR instead of commenting (per its docs: "If Codex has suggestions, it will comment; otherwise it will react with 👍"). A **usage-limit** message (`You have reached your Codex usage limits for code reviews`) means Codex did not review — treat as silence.
  - A reviewer is "heard from" this round only if its review is **round-bound** — new this round, not an old review of a previous SHA. **`commit_id` attestation covers review-objects and inline comments, but NOT issue comments (Claude) or reactions (Codex 👍).** For these, use a round-bound completion signal:
    - **`claude[bot]`** (issue comment): the comment's `updated_at` must be **after `ROUND_START_TS`** (step 4 captured it). `Claude finished` marker + post-round timestamp = heard from. An old `Claude finished` from a previous round does NOT count — compare timestamps.
