...
- **TDD evidence gate for BEHAVIORAL claims.** Attempt an executable reproduction **in a disposable scratch worktree** (the main working tree stays untouched — never mutate it from triage). Run it and read the result. Outcomes:
  - **Reproduces** (red on the current code, for the right reason, against a real oracle) → supports `FIX`; carry the test patch into step 7 as its reproducing test.
  - **Fails, and the attempted environment/inputs cover the claim and positively disprove it** → `HALLUCINATION`.
  - **Fails but does NOT positively disprove the claim** (no deterministic test seam; needs prod-only dep, permissions, version skew, fake clock, fault injection, or a test DB the cycle can't provide) → `UNVERIFIED` — record what reproducer/environment is missing. **Never** turn "couldn't reproduce" into `HALLUCINATION` by default — `HALLUCINATION` is reserved for a material claim that is *demonstrably false* (positive disproof), not for "we couldn't reproduce it".
