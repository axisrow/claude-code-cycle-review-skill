...
- **(cloud only) No reviewer was actually heard from this round** → do NOT finalize. If the configured bots posted nothing new for this round (the bots were slow, or Claude shows a usage-limit message instead of a review and no other reviewer responded), then no review happened — "no `FIX` verdicts" only reflects silence, not an approval. **How to recognize each bot's review** (studied from real PR comments — these are the markers to look for in step 6's fetched comments/reviews):
  ...
  - If at least one configured reviewer is heard from (round-bound), proceed. If nobody reviewed, notify the user and stop. This must be checked BEFORE interpreting the absence of `FIX` verdicts. (In local mode the equivalent is step 4's `/review` fail-closed.)
...
- **At least one `FIX`, and this is the 3rd cycle** → STOP, do not start a 4th. Three full review rounds with findings still outstanding means the PR isn't converging on its own — looping further wastes review budget. **Before handing back**, diagnose: are these findings variants of the same root cause? If so, the previous fixes patched symptoms, not the invariant — tell the user what invariant to fix. If the findings reveal a structural contradiction in the design, propose **simplification/refactoring** instead of more patches. Notify the user: summarize the still-open `FIX` findings, the diagnosed root cause (or design contradiction), and propose narrowing scope — e.g. move some findings **out of scope** into a follow-up issue/PR, or rethink the approach. Wait for the user's decision; do not merge and do not auto-loop. (Count a "cycle" as one completed round of steps 4–8, i.e. one review request + triage. The round that produced this 3rd batch of `FIX`s is the 3rd.)
...
**0. Diagnose the root cause — fix the invariant, not the symptom.** Before test-first, ask: *what invariant/contract does this bug violate?* Don't just close the described case — fix the invariant so that ALL variants of this bug class are closed at once. If you patch a symptom, the reviewer will find a new variant next round, and you'll burn cycles. Example: "empty identity → OWN_PR=true" — the invariant is "authorization defaults to denied, never granted on missing data". Fixing the invariant (single compound condition) closes ALL fail-open variants in one pass, instead of chasing each one across 3 cycles.

**If this is cycle 2+ and the new finding is a variant of a previous round's bug** — stop and reconsider: the previous fix patched a symptom, not the root cause. Re-diagnose the invariant; fix it properly this time. If the findings keep revealing the same structural contradiction across rounds, that's a signal the **design itself is wrong** — it needs refactoring/simplification, not more patches. Ask: *"What does this incident mean for the design as a whole?"* — if the answer reveals a contradiction (e.g., "a prompt-only skill can't be a security boundary"), simplify the design instead of adding more gates.
